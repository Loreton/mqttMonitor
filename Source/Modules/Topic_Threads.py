...
######################################################
def payload_worker(topic, payload, device):
    ...
    if prefix == 'tele' and suffix=='STATE' \
            or prefix == 'tele' and suffix=='HASS_STATE' \
            or prefix == 'stat' and suffix.startswith('STATUS') \
            or prefix == 'stat' and suffix=='RESULT':
        fUPDATE_file=True
...
def process(topic, payload, mqttClient_CB):
    if topic.startswith("tasmota/discovery"): ### topic='tasmota/discovery/DC4F22D3B32F/sensors'
    ...
    if topic_name in devices:
        ...
        funcArgs={'topic': topic, 'payload': payload, 'device': device}
    ...
    else: ### create entry for device name
        ...
        return # usciamo in modo a raccogliere maggiori info


    # elif prefix=="LnCmnd" and suffix=='query': ### topic='LnCmnd/telegram/topic_name/query'
    #     from_telegram_command(topic=topic, payload=payload, devices=devices)
    #     topic=None